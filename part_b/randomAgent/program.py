...
class Agent:
    ...
    def action(self, **referee: dict) -> Action:
        ...
        match self._color:
            case PlayerColor.RED:
                # choose random frog
                frog = random.choice(list(redValidMoves.keys()))

                # choose random action from redValidMoves
                move = random.choice(redValidMoves[frog])

                if move == "Grow":
                    return GrowAction()
                else:
                    return MoveAction(Coord(frog[0], frog[1]), move)

            case PlayerColor.BLUE:
                # choose random action from blueValidMoves
                frog = random.choice(list(blueValidMoves.keys()))
                move = random.choice(blueValidMoves[frog])
